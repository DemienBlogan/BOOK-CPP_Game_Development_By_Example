...
void DrawCommandBuffer::createCommandPool()
{
	...
	if (vkCreateCommandPool(VulkanContext::getInstance()->getDevice()->logicalDevice,
		&cpInfo, nullptr, &commandPool) != VK_SUCCESS)
...
void DrawCommandBuffer::allocateCommandBuffers(size_t imageCount)
{
	...
	if (vkAllocateCommandBuffers(VulkanContext::getInstance()->getDevice()->logicalDevice,
		&cbInfo, commandBuffers.data()) != VK_SUCCESS)
